...
def run_pipeline():
    logger.info("Iniciando o Pipeline ETL LuminaText...")
    ...
    for vacancy in raw_vacancies:
        logger.info(f"Processando vaga ID: {vacancy.id} - {vacancy.title}")
    ...
    if processed_documents:
        try:
            ...
            logger.info(f"Sucesso! {len(result.inserted_ids)} vagas processadas e salvas no MongoDB.")
        except Exception as e:
